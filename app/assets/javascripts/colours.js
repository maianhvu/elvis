$(function() {
  ...
  var colourNames = Object.keys(materialColours);

  // export colours to global namespace
  window['materialColour'] = function(name, shade) {
    if (!materialColours[name]) return undefined;
    if (!materialColours[name][shade]) return materialColours[name][500];
    return materialColours[name][shade];
  }

  // helper methods
  var moduleHashes = {};
  var primeMultiplier = function(index) {
    var mults = [127,31,7,257];
    if (index < 0) index += mults.length;
